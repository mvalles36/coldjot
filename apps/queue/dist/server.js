...
var import_database = require("@mailjot/database");
...
var QueueService = class {
  constructor() {
    ...
    this.sequenceQueue = new import_bull.default("sequence-processing", queueConfig);
    this.emailQueue = new import_bull.default("email-sending", queueConfig);
    this.schedulingQueue = new import_bull.default("scheduling", queueConfig);
    this.monitoringQueue = new import_bull.default("monitoring", queueConfig);
    this.cleanupQueue = new import_bull.default("cleanup", queueConfig);
...
// src/types/queue.ts
var import_types = require("@mailjot/types");

// src/lib/scheduling-service.ts
var import_types2 = require("@mailjot/types");
...
var SchedulingService = class {
  constructor() {
    this.defaultRateLimits = {
      ...
      perSequence: 1e3,
      cooldown: {
        afterBounce: 24 * 60 * 60 * 1e3,
        // 24 hours
        afterError: 15 * 60 * 1e3
        // 15 minutes
      }
    };
  }
  calculateNextRun(currentTime, step, businessHours, rateLimits = this.defaultRateLimits) {
    try {
      if (!businessHours) {
  ...
  calculateNextRunWithoutBusinessHours(currentTime, step, rateLimits) {
    const baseDelay = this.calculateBaseDelay(step);
    return addMinutes(currentTime, baseDelay);
  }
  calculateNextRunWithBusinessHours(currentTime, step, businessHours, rateLimits) {
  ...
  isWithinBusinessHours(date, businessHours) {
    ...
    if (businessHours.holidays.some(
      (holiday) => format(holiday, "yyyy-MM-dd") === dateString
    )) {
      return false;
    }
    const timeString = format(date, "HH:mm");
    return timeString >= businessHours.workHours.start && timeString <= businessHours.workHours.end;
  }
  getNextBusinessDay(date, businessHours) {
    let nextDay = date;
    let attempts = 0;
    const maxAttempts = 14;
    while (attempts < maxAttempts) {
      nextDay = addDays(nextDay, 1);
      if (businessHours.workDays.includes(nextDay.getDay()) && !businessHours.holidays.some(
        (holiday) => format(holiday, "yyyy-MM-dd") === format(nextDay, "yyyy-MM-dd")
      )) {
        return nextDay;
      }
      attempts++;
  ...
  distributeLoad(jobs, window, limits = this.defaultRateLimits) {
    try {
      ...
      const maxJobsForWindow = Math.min(
        ...
        Math.floor(windowDuration / (60 * 1e3) * limits.perMinute)
        // Convert window duration to minutes and multiply by per-minute limit
      );
  ...
  calculateBaseDelay(step) {
    switch (step.stepType) {
      case import_types.StepType.WAIT:
      ...
      case import_types.StepType.MANUAL_EMAIL:
      case import_types.StepType.AUTOMATED_EMAIL:
        switch (step.timing) {
          case import_types2.TimingType.IMMEDIATE:
            return 0;
          case import_types2.TimingType.DELAY:
            return step.delayAmount || 60;
          default:
            return 30;
        }
...
// src/server.ts
var import_types3 = require("@mailjot/types");
var app = (0, import_express.default)();
var port = process.env.PORT || 3001;
var queueService = new QueueService();
var schedulingService2 = new SchedulingService();
app.use((0, import_cors.default)());
app.use(import_express.default.json());
async function getBusinessHours(userId) {
  const settings = await import_database.prisma.businessHoursSettings.findFirst({
    where: { userId }
  });
  if (!settings) {
    return void 0;
...
async function getFirstSequenceStep(sequenceId) {
  const step = await import_database.prisma.sequenceStep.findFirst({
    where: {
      sequenceId,
      previousStepId: null
  ...
  return {
    ...
    status: step.status.toUpperCase() === "NOT_SENT" ? import_types3.StepStatus.NOT_SENT : step.status,
...
app.post("/api/sequences/:id/process", async (req, res) => {
  try {
    ...
    const nextRun = schedulingService2.calculateNextRun(
      /* @__PURE__ */ new Date(),
      firstStep,
      businessHours
    );
